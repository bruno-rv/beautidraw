...
def render(
    ...
    try:
        ...
        print("Run: cd <repo-root>/scripts/qa && uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    vp_height = min(int(max(diagram_h, 600)), 16000)

    # Output path
    if output_path is None:
        output_path = excalidraw_path.with_suffix(".png")

    # Template path (same directory as this script, or env override)
    template_path = Path(os.environ.get(
        "EXCALIDRAW_RENDER_TEMPLATE",
        str(Path(__file__).parent / "render_template.html")))
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print("Run: cd <repo-root>/scripts/qa && uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
        ...
        page.wait_for_function("window.__renderComplete === true", timeout=120000)
        ...
        try:
            svg_el.screenshot(path=str(output_path), timeout=60000, animations="disabled", scale="css")
        except Exception:
            box = svg_el.bounding_box()
            page.set_viewport_size({"width": min(int(box['width']), 16000), "height": min(int(box['height']), 16000)})
            page.screenshot(path=str(output_path), clip=box, animations="disabled")
        browser.close()
